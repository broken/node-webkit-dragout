...
- (void)fillPasteboard {
  ...
  if (_dropData->text && !_dropData->text->empty()) {
    NSString *tmp = SysUTF16ToNSString(*_dropData->text);
    ...
    if (filenameTokenLoc.location != NSNotFound) {
      ...
      if ([filenames count] > 0) {
        _dragOperationMask &= ~NSDragOperationMove;
        [_pasteboard setPropertyList:filenames forType:NSFilenamesPboardType];
      }
    } else {
