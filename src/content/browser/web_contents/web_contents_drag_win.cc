...
HRESULT DragFilesToDesktop(HWND window, std::vector<string16> filenames);
...
bool WebContentsDragWin::DoDragging(const DropData& drop_data,
	...
	if (!drop_data.text.string().empty()) {
		const string16& tmp = drop_data.text.string();
		const char16 kExtensisFilenamesToken[] = { 'e', 'x', 't', 'e', 'n', 's', 'i', 's', '-', 'f', 'i', 'l', 'e', 'n', 'a', 'm', 'e', 's', '-', 't', 'y', 'p', 'e', ':', 0 };
		const char16 kFilenamesDelimiterToken[] = { '|', '\n', '|', 0 };
		std::vector<string16> filenames;

		if (tmp.find(kExtensisFilenamesToken) == 0) {
			size_t pos = string16(kExtensisFilenamesToken).length();
			while (pos != string16::npos) {
				string16 filename;
				size_t pos2 = tmp.find(kFilenamesDelimiterToken, pos);

				if (pos2 == string16::npos) {
					...
					filenames.push_back(filename);
					pos = pos2 + string16(kFilenamesDelimiterToken).length();
				}
			}
			...
			if(SUCCEEDED(hr)) {
				...
				// Continue with default d&d behavior. This will drag thumbnail images for selected items. 
				// TODO We may wish to display an error message instead.
			}
		}
	}
	// END EXTENSIS
...
public:
	...
	CExtDataObject(std::vector<string16> paths);
	~CExtDataObject();

private:

	int LookupFormatEtc(FORMATETC *pFormatEtc);
	HGLOBAL CreateDropfiles(std::vector<string16>& paths);

	std::vector<string16> m_paths;
	LONG	   m_refCount;
	FORMATETC  m_formatEtc;
};

CExtDataObject::CExtDataObject(std::vector<string16> paths) {
...
HRESULT CExtDataObject::GetData(FORMATETC *formatEtc, STGMEDIUM *stgMedium) {
	std::vector<string16> pathsThatExist;
	...
	case TYMED_HGLOBAL:
		...
		for(std::vector<string16>::iterator i = m_paths.begin(); i != m_paths.end(); i++) {
...
HGLOBAL CExtDataObject::CreateDropfiles(std::vector<string16>& paths) {
	...
	for(std::vector<string16>::iterator i = paths.begin(); i != paths.end(); i++) {
...
HRESULT DragFilesToDesktop(HWND window, std::vector<string16> paths) {
	HRESULT hr = S_OK;
